...
class StringMatcher:
    ...
    def preciseBatchMatch(self, path, min_similarity, outputfilename, gold_mapping_path, kno=5, max_similarity=1.0):
        ...
        for nodeid in self.src_graphmanager.graph.keys():
            ...
            for src_prop in self.src_graphmanager.indices.keys():
                for tgt_prop in self.tgt_graphmanager.indices.keys():
                    try:
                        ...
                        cind = self.tgt_graphmanager.indices[tgt_prop].getIndicesForValue(x)
            ...
            if len(indices)<1:
                ...
                matchfound=False
                best_matching_resources = heapq.nlargest(100,tmp_tgt_ind.items(), key=operator.itemgetter(1)) #max((tmp_tgt_ind.items()), key=operator.itemgetter(1))

                bmr = pd.DataFrame(np.array(best_matching_resources))
                try:
                    bmr[2] = bmr.apply(lambda row: len(self.tgt_graphmanager.graph[row[0]][list(self.tgt_graphmanager.indices.keys())[0]]), axis=1)
                except:
                    bmr[2] = bmr.apply(lambda row: len(self.tgt_graphmanager.graph[row[0]][list(self.tgt_graphmanager.indices.keys())[1]]), axis=1)
                bmr[1] = bmr[1].astype('int32')
                bmr[2] = bmr[2].astype('int32')
                bmr.sort_values(by=[1,2], ascending=[False, True], inplace=True, axis=0)
                bmr = bmr.head(kno)
                ...
                for index, row in bmr.iterrows():
                    ...
                    for tgt_prop in self.tgt_graphmanager.indices.keys():
                        try:
                            y = self.tgt_graphmanager.graph[best_matching_resource[0]][tgt_prop]
                    ...
                    if best_matching_resource[1] > no_of_ngrams*min_similarity:# and best_matching_resource[1] <= min(maximum_ngrams_x, maximum_ngrams_y)*max_similarity:
                            try:
                                    correspondences[nodeid] = best_matching_resource[0]
                                    srcid = str(nodeid).replace("<","").replace(">","")
                                    tgtid = str(best_matching_resource[0]).replace("<","").replace(">","")
                                    if len(gold_mapping.loc[(gold_mapping.src_id==srcid) & (gold_mapping.tgt_id==tgtid)])>0:
                                        lbl=1
                                    else:
                                        lbl=0
...
if __name__ == '__main__':
    ...
    #["he","dc"],["ma","dc"],
                ...
                #["ru","da"],["ru","ol"],
    #prefixes =[["darkscape","oldschoolrunescape"]]
    #prefixes =[["hilti_erp","hilti_web"]]

    labelspath = "C:/Users/Alexander/rdata2graph/data/oaei_data/labels.txt"
    #labelspath = "C:/Users/Alexander/rdata2graph/data/sap_hilti_data/labels.txt"
    kno = 5
    outputfilename = "oaei_gold_standard"+str(kno)+"best.csv"
    for prefix in prefixes:
        basedir = "C:/Users/Alexander/rdata2graph/data/"+prefix[0]+prefix[1]+"/"
        #basedir = "C:/Users/D072202/rdata2graph/data/sap_hilti_data/"
        try:
            os.remove(basedir+outputfilename)
        ...
        #gold_mapping_path = basedir + "gold_standard.csv"
        #gold_mapping_path = basedir + "final_testset.csv"
        gold_mapping_path = basedir + prefix[0]+prefix[1]+".csv"
        mappings = stringmatcher.preciseBatchMatch(basedir+outputfilename, 0.7, outputfilename, gold_mapping_path, kno)
