...
class StringMatcher:
    ...
    def preciseBatchMatch(self, path, min_similarity, max_similarity=1.0, keys=None,x=0):
...
def main(keys, q, x):
    q.put("Starting to match on thread")
    try:
        os.remove("../../data/oaei_data/oaei_gold_standard.csv"+str(x))
    ...
    mappings = stringmatcher.preciseBatchMatch("../../data/oaei_data/oaei_gold_standard.csv", 0.9, 1.0, keys, x)
