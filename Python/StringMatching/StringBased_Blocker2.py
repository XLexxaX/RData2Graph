...
class StringMatcher:
    ...
    def preciseBatchMatch(self, path, min_similarity, outputfilename, gold_mapping_path, kno=5, max_similarity=1.0):
        ...
        correspondences = dict()
        i=0
        fulllen = len(self.src_graphmanager.graph.keys())
        i=0
        total_size = len(self.src_graphmanager.graph.keys())
        for nodeid in self.src_graphmanager.graph.keys():
            mlx = len(nodeid)*min_similarity
            bmr = dict()
            smallest = 0
            i=i+1
            if i%10==0:
                print("  " + str(int(100*i/(total_size))) + "% done")
            for nodeid2 in self.tgt_graphmanager.graph.keys():

                for src_prop in self.src_graphmanager.indices.keys():
                    for tgt_prop in self.tgt_graphmanager.indices.keys():
                        x = self.src_graphmanager.graph[nodeid][src_prop]
                        y = self.tgt_graphmanager.graph[nodeid2][tgt_prop]
                        tmp = len([v for v in x if v in y])
                        if tmp > mlx:
                                    bmr[tmp] = nodeid2
                                    if len(gold_mapping.loc[(gold_mapping.src_id==nodeid) & (gold_mapping.tgt_id==nodeid2)])>0:
                                        lbl=1
                                    else:
                                        lbl=0
...
if __name__ == '__main__':
    ...
    #labelspath = "C:/Users/D072202/RData2Graph/rdata2graph/data/sap_hilti_data/labels.txt"
    kno = 5
    outputfilename = "oaei_gold_standard"+str(kno)+"best.csv"
    for prefix in prefixes:
        basedir = "C:/Users/D072202/RData2Graph/rdata2graph/data/"+prefix[0]+prefix[1]+"/"#sap_hilti_data/"#+prefix[0]+prefix[1]+"/"
        #basedir = "C:/Users/D072202/RData2Graph/rdata2graph/data/sap_hilti_data/"
        try:
        ...
        mappings = stringmatcher.preciseBatchMatch(basedir+outputfilename, 0.5, outputfilename, gold_mapping_path, kno)
